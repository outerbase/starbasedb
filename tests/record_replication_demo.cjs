...
async function main() {
  console.log("Running replication plugin tests...");
  let testOutput = "";
  try {
    testOutput = execSync("npx vitest run plugins/replication", { encoding: "utf-8" });
  } catch (error) {
...
<head>
  <meta charset="UTF-8">
  <title>StarbaseDB Replication Plugin Tests</title>
  <style>
...
<body>
  <div class="container">
    <div class="header">
      <div class="title">StarbaseDB Replication Plugin</div>
    ...
    <div class="terminal" id="term">
      <span class="comment">// Executing replication plugin vitest suite...</span><br>
      <span class="success-text">npx vitest run plugins/replication</span><br><br>
  ...
  <script>
    ...
    // Simulate real-time typing of test results
    function typeWriter() {
      if (i < output.length) {
  ...
  const htmlPath = path.join(assetsDir, "replication_tests.html");
  ...
  try {
    ...
    // Wait for typing animation to complete + extra view time (total ~12 seconds)
    ...
    if (videoFile) {
      ...
      const newPath = path.join(assetsDir, "replication_demo.webm");
